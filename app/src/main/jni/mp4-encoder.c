//
// Created by Coder on 2019-07-23.
//
...
open_audio(AVFormatContext *oc, AVCodec *codec, OutputStream *ost, AVDictionary *opt_arg) {
    ...
    av_opt_set_int(ost->swr_ctx, "in_sample_rate", 44100, 0);
    av_opt_set_sample_fmt(ost->swr_ctx, "in_sample_fmt", AV_SAMPLE_FMT_S16P, 0);
 ...
 * @return
 */
AVFrame *get_video_frame(OutputStream *ost, uint8_t *data, AndroidBitmapInfo info) {
    AVCodecContext *c = ost->enc;
...
//        return NULL;
    ...
    if (c->pix_fmt != AV_PIX_FMT_YUV420P) {
        ...
        ARGBToI420(data, c->width * 4,
                   ost->tmp_frame->data[0], c->width,
                   ost->tmp_frame->data[1], (c->width + 1) / 2,
                   ost->tmp_frame->data[2], (c->width + 1) / 2,
                   c->width, c->height);
 ...
 */
int write_video_frame(uint8_t *data, AndroidBitmapInfo info) {
    ...
    frame = get_video_frame(ost, data, info);
...
//        frame->pts = av_rescale_q(ost->samples_count, (AVRational) {1, ost->frame->sample_rate},
//                                  c->time_base);
//        ost->samples_count += ost->frame->sample_rate;
    } else {
        return -10000;
    }
